...
BEGIN
  -- Total aligners still planned across active TreatmentPlan cases.
  ...
  WITH calc AS (
    SELECT
      ...
      b.unit,
      b.pack_size,
      -- per_case materials scale with the active-case count; everything
      -- else scales with aligners remaining to produce.
      CEIL(
        b.qty_per_aligner
